...
const LotteryImages = () => {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          let lotteryImages = websiteData?.lotteryImages?.filter(
            (item) => item !== id
          );
          const mainData = {
            lotteryImages,
  ...
  const handle = async () => {
    ...
    if (websiteData?.lotteryImages?.length > 0) {
      lotteryImages = [lotteryImg, ...websiteData?.lotteryImages];
    } else {
      lotteryImages = [lotteryImg];
    }
  ...
  return (
    <div className="md:mt-5">
      <div className="flex justify-between items-center px-2">
        <h2 className="text-white text-2xl font-bold my-5">Lottery Images</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 items-center lg:grid-cols-4 gap-5">
        {websiteData?.lotteryImages?.map((item, idx) => (
          <div key={idx}>
            <Image
              src={item}
              height={500}
